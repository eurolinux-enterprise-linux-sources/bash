...
Patch016: ftp://ftp.gnu.org/pub/gnu/bash/bash-4.1-patches/bash41-016
Patch017: ftp://ftp.gnu.org/pub/gnu/bash/bash-4.1-patches/bash41-017
...
#1007926, fix printing of multiline here-strings
Patch133: bash-4.1-here-strings.patch

#951171, fix double free after SIGCHLD
Patch134: bash-4.1-signal-sarestart.patch

#1012015, fix certain brace expansion cases
Patch135: bash-4.1-brace-expansion.patch

#948207, fix file descriptor leaks
Patch136: bash-4.1-fd-leaks.patch

#986095, bash hangs/crashes with some history settings
Patch137: bash-4.1-history-hang.patch

#1011064, describe changes in extglob behaviour
Patch138: bash-4.1-extglob-man.patch

#1010164, fix some heavy memory leaks
Patch139: bash-4.1-mem-leaks.patch

#1102803, fix double echoes
Patch140: bash-4.1-noecho.patch

#1141646
Patch141: bash-4.1-env-inject.patch

# 1146322 - cve-2014-7169

Patch142: bash-4.2-cve-2014-7169-0.patch
Patch143: bash-4.2-cve-2014-7169-1.patch
Patch144: bash-4.2-cve-2014-7169-2.patch

Patch145: bash-4.1-posix-block-size-for-cf-options.patch
#1148507
...
%patch132 -p1 -b .trap
%patch133 -p1 -b .here-strings
%patch134 -p1 -b .sarestart
%patch135 -p1 -b .brace-expansion
%patch136 -p1 -b .fd-leaks
%patch137 -p1 -b .hang
%patch138 -p1 -b .extglob-man
%patch139 -p1 -b .mem-leaks
%patch140 -p1 -b .noecho
%patch141 -p0 -b .inject
%patch142 -p0 -b .7169-0
%patch143 -p0 -b .7169-1
%patch144 -p0 -b .7169-2
...
* Fri Jan 16 2015 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-32
- Allow importing exported functions with hyphens
  Resolves: #1148507

* Fri Jan 16 2015 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-31
- Better doc. for ulimit -c and -f options
  Resolves: #1119587

* Fri Jan 16 2015 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-30
- Fix a segfault from shellshock check-scripts
  Resolves: #1150544

* Fri Sep 26 2014 Michal Hlavinka <mhlavink@redhat.com> - 4.1.2-29
- CVE-2014-7169
  Resolves: #1146323

* Mon Sep 15 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-28
- Fix-up the patch
  Related: #1141646

* Mon Sep 15 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-27
- Check for fishy environment
  Related: #1141646

* Wed Jul 24 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-26
- Final fix for bash vi mode
  Related: #1102803

* Tue Jun 10 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-25
- Fix double echoes differently
  Related: #1102803

* Tue Jun 10 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-24
- Fix memory leaks and keep coverity happy
  Related: #1010164

* Tue Jun 10 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-23
- Fix double echoes
  Resolves: #1102803

* Fri May 30 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-22
- Fix some heavy memory leaks
  Resolves: #1010164

* Wed May 28 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-21
- Document changes to the extglob behaviour in the man page
  Resolves: #1011064

* Tue May 27 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-20
- Fix reading of empty history
  Resolves: #986095

* Mon May 26 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-19
- Fix file descriptor leaks
  Resolves: #948207

* Mon May 26 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-18
- Fix certain brace expansion cases
  Resolves: #1012015

* Mon May 26 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-17
- Fix double free after receiving SIGCHLD
  Resolves: #951171

* Fri Feb 21 2014 Ondrej Oprala <ooprala@redhat.com> - 4.1.2-16
